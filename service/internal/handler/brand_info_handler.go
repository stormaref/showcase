...
type BrandInfoHandler struct {
	brand *service.BrandInfoService
}

func NewBrandInfoHandler(brand *service.BrandInfoService) *BrandInfoHandler {
	return &BrandInfoHandler{brand: brand}
}

func (h *BrandInfoHandler) GetPublic(c *gin.Context) {
	loc := locale.Get(c)
	item, err := h.brand.GetPublic(c.Request.Context(), loc)
	if err != nil {
		if errors.Is(err, repository.ErrBrandInfoNotFound) {
...
func (h *BrandInfoHandler) GetAdmin(c *gin.Context) {
...
func (h *BrandInfoHandler) Update(c *gin.Context) {
	var in service.BrandInfoUpdateInput
